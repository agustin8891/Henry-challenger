...
function stringMasLarga(str) {
  ...
  for(let i = 1; i < array.length; i++) {
    if(maximo.length < array[i].length){
	    maximo = array[i];
    }
  }
  return maximo;
...
function actividadesEnComun(persona1, persona2) {
  ...
  for(let i = 0; i < persona1.length; i++) {
	  for(let t = 0; t < persona2.length; t++){
		  if (persona1[i]===persona2[t]){
			  array.push(persona1[i])
      }
    }		
  }
  return array;
...
function sumarElTipo(arreglo) {
  ...
  for(let i = 0; i < arreglo.length; i++){
	  ...
	  for(let t = 0; t < arreglo.length; t++){
		  if (arreglo[i]===arreglo[t]) {
			  cont++;
		  }
	  }
	objeto[arreglo[i]]=cont;
  }
  return objeto;
